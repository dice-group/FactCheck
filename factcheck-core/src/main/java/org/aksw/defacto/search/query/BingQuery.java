...
public class BingQuery implements Query {
    ...
    public String generateQuery(MetaQuery query) {
        ...
        if ( query.getTopicTerms().isEmpty() ) {
            ...
            if ( query.getPropertyLabel().equals("??? NONE ???") ) queryString = String.format("\"%s %s\"", subject, object);
            else queryString = String.format("\"%s %s %s\"", subject, property, object);
        }
        else {
            
            // this query is going to be exectued from the topic majority web feature
            queryString = String.format("\"%s\" \"%s\" \"%s\"", subject, property, object);
